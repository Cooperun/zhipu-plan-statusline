...
- **5h / 7d**：智谱主套餐的两个滚动 token 窗口用量百分比（满了会被限速）。绿 <60% / 黄 60–85% / 红 >85%。括号里是**距重置的倒计时**（如 `1h23m`、`5d8h`）。
...
- ⚠️ **老式 cmd.exe（传统 conhost）**：脚本会**自动降级**成纯文本（`GLM(Pro) 5h:[##--------]9% (1h23m)`），数字照样准，只是没有颜色和方块。建议还是用 Windows Terminal，体验更好。
...
| `5h: ██░ 31% (1h23m)` | **5 小时**滚动 token 窗口已用百分比，括号内是距重置倒计时，满了会限速 |
| `7d: ██░ 11% (5d8h)` | **7 天**滚动 token 窗口已用百分比，括号内是距重置倒计时 |
